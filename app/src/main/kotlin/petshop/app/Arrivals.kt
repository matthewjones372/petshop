...
/** The background work every service has one of: new pets keep turning up. The run answers how many. */
 ...
 * future nobody read, beside the way to stop it when the graph is given back. It names no backend: it
 * runs on whatever the graph binds.
 */
val arrivals: Module =
    singleOf(
        { ref: ActorRef<Shop>, config: Settings, streams: StreamBackend ->
            ...
            Stream.tick(every = config.arrivalsEvery, element = Unit)
                ...
                .runFold(0L) { arrived, pet ->
                    // Written on the run's own thread, not a request's: the pair is on the line because
